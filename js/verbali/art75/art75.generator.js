...
import { $, normPeso, ensureDotEnd, ensureEndsVerbaleMark, renderHeader, getSoggetto, getFormattedItalianDate, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti, renderDivider } from '../../core/utils.js';
import { testoDifensore } from '../../core/difensore.js';
import { conc, qualificaPG } from '../../core/militari.js';
import { renderFirmaComandante } from '../../core/gruppo-firma.js';
...
export function generaVerbale75(getOperantiListFn){
  ...
  const corpoStd = `In data ${dataVerbale}, alle ore ${oraVerbale}, ${luogoVerbale}, ${conc("il sottoscritto", "i sottoscritti")} ${qualificaPG("estesa")} ${operanti}, in forza al Comando in intestazione, danno atto di quanto segue.------//`;
  ...
  // Generalità del proprietario diverso dal trasgressore, composte come per
  // ogni altro soggetto del sito.
  const ownerOther = (() => {
    const v = (id) => (document.getElementById(id)?.value || "").trim();
    const cogn = v("veh_prop_cognome"), nome = v("veh_prop_nome");
    if(!cogn && !nome) return "";
    const F = document.getElementById("veh_prop_sesso")?.value === "F";
    const natoA = v("veh_prop_nato_a"), natoIl = v("veh_prop_nato_il");
    const resCom = v("veh_prop_res_comune"), resVia = v("veh_prop_res_via"), resCiv = v("veh_prop_res_civ");
    let t = `${cogn} ${nome}`.trim();
    if(natoA || natoIl) t += `, nat${F ? "a" : "o"} a ${natoA || "____________"} il ${natoIl || "____________"}`;
    if(resCom) t += `, residente a ${resCom}${resVia ? ` in ${resVia}` : ""}${resCiv ? ` ${resCiv}` : ""}`;
    return t;
  })();
  ...
  html += renderSignatureBlock([s1.isFemale ? "La Trasgreditrice" : "Il Trasgressore", bloccoVerbalizzanti()]);
...
export function generaTrasmissioneHTML(){
  ...
  const comandante = (document.getElementById("trasm_comandante")?.value || "").trim();
  ...
  html += `
    <div style="margin-top:35pt; text-align:center; margin-left:auto; margin-right:auto; width:220pt; ${fontMain}">
      ${renderFirmaComandante(comandante)}
    </div>
  `;
...
export function generaPerquisizione(getOperantiListFn){
  ...
  html += pj(`In data ${dataVerbale}, alle ore ${oraStesura}, ${luogo}, ${conc("il sottoscritto", "i sottoscritti")} ${qualificaPG("estesa")} ${operanti}, in forza al Comando in intestazione, danno atto di quanto segue.`);
  ...
  html += pj(`${Nominato}, preliminarmente ${avvertito} della facolt\u00e0 di farsi assistere da un difensore o da persona di fiducia prontamente reperibile, dichiara:`);
  html += pj(testoDifensore("perq"));
  ...
  const colonne = [s1.isFemale ? "L'Interessata" : "L'Interessato", bloccoVerbalizzanti()];
...
export function generaNarcotest(getOperantiListFn){
  ...
  html += renderSignatureBlock([F ? "L'Interessata" : "L'Interessato", bloccoVerbalizzanti()]);
