...
import { getCurrentPratica, getPraticheAperte, showLock, showAppPratica, attivaPratica } from './auth.js';
import { getOrdineSegnalibri, applicaOrdineSegnalibri } from './segnalibri.js';
import { serializzaIstanze161, ripristinaIstanze161 } from '../verbali/art161/istanze.js';
...
export function collectDraft(docAttivo){
  ...
  return {
    ...
    // Le istanze del verbale ex art. 161 (autonomo, da S.I.T., da
    // perquisizione art. 4) sono atti distinti e viaggiano insieme alla bozza.
    istanze161: serializzaIstanze161(),
    // Durante un intervento si redigono più verbali sulle stesse generalità:
    // la bozza conserva quindi l'intero elenco delle pratiche in lavorazione.
    praticheAperte: getPraticheAperte(),
    ordineSegnalibri: getOrdineSegnalibri(),
    fields,
    operanti,
    sostanzeExtra,
    sitQA,
...
export function applySavedDraft(draft, syncHelpers) {
  ...
  // Riapre tutte le pratiche che erano in lavorazione, riportando in primo
  // piano quella che si stava compilando.
  const aperte = Array.isArray(draft.praticheAperte) && draft.praticheAperte.length
    ? draft.praticheAperte
    : (draft.currentPratica ? [draft.currentPratica] : []);
  aperte.forEach(t => showAppPratica(t));
  if(draft.currentPratica) attivaPratica(draft.currentPratica);
  applicaOrdineSegnalibri(draft.ordineSegnalibri);
  ripristinaIstanze161(draft.istanze161);
  ...
  if (typeof syncHelpers.syncSopralluogoUI === "function") syncHelpers.syncSopralluogoUI();
  if (typeof syncHelpers.syncVeicoloRestituzioneUI === "function") syncHelpers.syncVeicoloRestituzioneUI();
  if (typeof syncHelpers.syncAffidamentoUI === "function") syncHelpers.syncAffidamentoUI();
  if (typeof syncHelpers.syncNotificaUI === "function") syncHelpers.syncNotificaUI();
  if (typeof syncHelpers.syncFermoSeqUI === "function") syncHelpers.syncFermoSeqUI();
  if (typeof syncHelpers.syncSeqVeicoloUI === "function") syncHelpers.syncSeqVeicoloUI();
  if (typeof syncHelpers.syncRimozioneUI === "function") syncHelpers.syncRimozioneUI();
  if (typeof syncHelpers.syncPatente223UI === "function") syncHelpers.syncPatente223UI();
  if (typeof syncHelpers.syncPatenteIllUI === "function") syncHelpers.syncPatenteIllUI();
  if (typeof syncHelpers.syncPrelievoUI === "function") syncHelpers.syncPrelievoUI();
  if (typeof syncHelpers.syncTulps15UI === "function") syncHelpers.syncTulps15UI();
  if (typeof syncHelpers.syncInvito650UI === "function") syncHelpers.syncInvito650UI();
  if (typeof syncHelpers.syncCadavereUI === "function") syncHelpers.syncCadavereUI();
  if (typeof syncHelpers.ripristinaElenchiReati === "function") syncHelpers.ripristinaElenchiReati();
