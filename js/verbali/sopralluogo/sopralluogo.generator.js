...
import { $, renderHeader, getSoggetto, renderSignatureBlock, bloccoVerbalizzanti, splitItemsList, joinItemsWithSemicolons } from '../../core/utils.js';
...
export function generaSopralluogo(getOperantiListFn){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${conc("il sottoscritto", "i sottoscritti")} ${qualificaPG("sigla")} <b>${operanti}</b>, ${conc("effettivo al", "effettivi al")} Reparto in intestazione, durante servizio perlustrativo.`);
  ...
  html += renderSignatureBlock([F ? "L'Interessata presente" : "L'Interessato presente", bloccoVerbalizzanti()]);
