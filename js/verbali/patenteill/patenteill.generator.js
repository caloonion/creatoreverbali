...
import { $, renderHeader, getSoggetto, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti } from '../../core/utils.js';
...
export function generaPatenteIll(getOperantiListFn){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}, ${conc("il sottoscritto", "noi sottoscritti")} <b>${operanti}</b>, ${qualificaPG("breve")} in servizio presso l'ufficio di cui all'intestazione, ${conc("do atto", "diamo atto")} che in data e luogo di cui sopra, in ottemperanza alla circolare del Ministero dell'Interno nr. M/2413-15 emessa in data 04 ottobre 1999, ${conc("ho proceduto", "abbiamo proceduto")} al ritiro della patente di guida in oggetto indicata che, per lo stato di illeggibilit&agrave; e/o deterioramento di seguito specificato, non risulta pi&ugrave; idonea alla funzione di certificazione della titolarit&agrave; dell'abilitazione alla guida, nonch&eacute; a quella di identificazione personale del conducente del predetto veicolo.`);
