...
export function generaCadavere(getOperantiListFn){
  ...
  // Il luogo dell'intervento è composto da via e comune, cui può aggiungersi
  // una descrizione libera; se il verbale è redatto presso gli Uffici, via e
  // comune restano quelli della sezione Dati verbale.
  const luogoIntervento = (() => {
    const via = val("cad_via");
    const comune = val("cad_comune");
    const dettaglio = val("cad_luogo");
    const base = [via, comune].filter(Boolean).join(", ");
    if(base && dettaglio) return `${base} (${dettaglio})`;
  ...
  html += renderSignatureBlock(["Firma del Custode", bloccoVerbalizzanti()]);
