...
/**
 * Rende una fattispecie nella forma tentata: negli atti si richiama l'art. 56
 * c.p. in combinato disposto con la norma incriminatrice. Se il testo la cita
 * già, non viene raddoppiata.
 */
export function applicaTentativo(testo, attivo){
  const t = (testo || "").trim();
  if(!attivo || !t) return t;
  if(/\b56\b[^.]*codice penale/i.test(t) || /tentat/i.test(t)) return t;

  // Formula del tentativo: si richiama l'art. 56 c.p. in combinato disposto
  // con la norma incriminatrice e si descrive la condotta con la formula di
  // legge, senza volgere all'infinito la descrizione del reato consumato
  // (operazione che in italiano non ha una regola meccanica affidabile).
  const m = t.match(/^(.*?)\s*\((.*?)\)\s*,\s*per\s+/s);
  if(m){
    const articolo = m[1].trim();
    const rubrica = m[2].trim();
    return `56 e ${articolo} (${rubrica} &mdash; nella forma tentata), per aver compiuto atti idonei diretti in modo non equivoco a commettere il delitto di ${rubrica.toLowerCase()}, non riuscendo nell'intento per cause indipendenti dalla propria volont\u00e0`;
  }
  return `56 del codice penale in relazione all'art. ${t}, nella forma del tentativo`;
}
