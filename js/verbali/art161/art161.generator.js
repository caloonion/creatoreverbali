...
import { $, renderHeader, getSoggetto, getDocTipo, getLuogoVerbaleText, renderSignatureBlock } from '../../core/utils.js';
...
export function generaVerbale161(getOperantiListFn){
  ...
  // Se siamo in modalità SIT con interruzione, ereditiamo il reato se non compilato a mano
  ...
  const fatto = (document.getElementById("v161_fatto")?.value || "____________________________________________________________").trim();
  ...
  html += pj(`Il ${dataVerbale}, alle ore ${oraVerbale}, presso: ${luogoVerbale}, avanti al sottoscritto Ufficiale/Agente di Polizia Giudiziaria ${operanti}, effettivo al predetto reparto, è presente:`);
  ...
  if(isUfficio && domTipo==="elegge") {
    html += pj(`Dichiaro domicilio presso lo studio del difensore d'ufficio sopra menzionato, individuato secondo il turno, come disciplinato dalla vigente normativa; si procedeva a contattarlo attraverso: <b>${contattoMezzo}</b>, il quale riferiva di ${accettaUfficioSi ? `"accettare la domiciliazione"` : `"non accettare la domiciliazione"`}.`);
  ...
  html += pj(`Fatto, letto, confermato e sottoscritto, in data e luogo di cui sopra. Copia del presente verbale viene rilasciata ${indagatoTxt} per gli usi consentiti dalla legge.`);
