...
/**
 * Numero civico: se l'operatore digita il solo numero, il verbale deve
 * comunque leggersi "nr. 12". La dicitura non viene raddoppiata se è già
 * stata scritta a mano, in qualunque grafia ("n.", "n°", "nr").
 */
export function normalizzaCivico(testo){
  const s = (testo || "").trim().replace(/\s+/g, " ");
  if(!s) return "";
  // "snc" (senza numero civico) è una dicitura a sé: non va preceduta da "nr."
  if(/^s\.?n\.?c\.?$/i.test(s)) return "snc";
  const gia = s.match(/^(nr\.?|n°|n\.?|num\.?)\s*(.+)$/i);
  if(gia) return `nr. ${gia[2].trim()}`;
  return `nr. ${s}`;
}

/** Cognomi, numeri di documento, targhe e patenti: tutto in maiuscolo. */
export function maiuscolo(testo){
  return (testo || "").trim().replace(/\s+/g, " ").toUpperCase();
}

/**
 * Vie e piazze: si conserva quanto digitato, correggendo solo l'iniziale
 * delle parole. Non viene anteposta la parola "via", perché l'operatore la
 * scrive quasi sempre e si otterrebbe una ripetizione.
 */
export function normalizzaIndirizzo(testo){
  const s = (testo || "").trim().replace(/\s+/g, " ");
  if(!s) return "";
  return capitalizzaNome(s);
}
