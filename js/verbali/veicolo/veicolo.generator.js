...
import { $, renderHeader, getSoggetto, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti, splitItemsList, joinItemsWithSemicolons, getTipoDocumento , descrittoreDocumento} from '../../core/utils.js';
...
function getIncaricato(){
  ...
  const doc = getTipoDocumento("vei_inc_doc");
  const docNum = val("vei_inc_doc_num");
  ...
  if(tel) dati += `tel. ${tel}, `;
  if(doc){
    dati += `identificat${isFemale ? "a" : "o"} mediante ${doc}${docNum ? ` nr. ${docNum}` : ""}`;
...
export function generaVeicolo(getOperantiListFn){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}, ${conc("il sottoscritto", "noi sottoscritti")} ${qualificaPG("sigla")} <b>${operanti}</b>, ${conc("effettivo al", "effettivi al")} Reparto in intestazione, ${conc("do atto", "diamo atto")} di aver proceduto, a seguito del rinvenimento avvenuto alle precedenti ore <b>${oraRinvenimento || "______"}</b>${rinvMezzo ? ` a mezzo <b>${rinvMezzo}</b>` : ""}, alla restituzione del mezzo in oggetto indicato, risultato <b>${marciante ? "MARCIANTE" : "NON MARCIANTE"}</b> e provvisto di quanto segue:`);
