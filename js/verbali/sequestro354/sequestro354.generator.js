...
import { $, renderHeader, getSoggetto, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti, splitItemsList } from '../../core/utils.js';
...
export function generaSequestro354(getOperantiListFn, prefix = "seq354"){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}, ${conc("il sottoscritto", "noi sottoscritti")} <b>${operanti}</b>, ${qualificaPG("breve")} in servizio presso l'ufficio di cui all'intestazione, ${conc("rendo noto", "rendiamo noto")} a chi di dovere perché consti che in data e luogo di cui sopra ${conc("ho proceduto", "abbiamo proceduto")} al sequestro del materiale in oggetto indicato in quanto corpo di reato o cosa pertinente il reato.`);
