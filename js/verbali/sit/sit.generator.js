...
import { $, renderHeader, getSoggetto, getLuogoVerbaleText, ensureEndsVerbaleMark, renderSignatureBlock, bloccoVerbalizzanti, joinItemsWithSemicolons } from '../../core/utils.js';
...
export function generaSITHTML(getOperantiListFn){
  ...
  // Le fattispecie emerse sono un elenco, ciascuna con la propria ipotesi di
  // tentativo.
  const reatiSit = getReatiFormattati("sit");
  const reatoEmergenti = reatiSit.length
    ? joinItemsWithSemicolons(reatiSit)
  ...
  if(interruzioneOn){
    html += pj(`<b>A questo punto, emergendo indizi di reit&agrave; a carico della persona esaminata in ordine ${reatiSit.length > 1 ? "ai reati ex Artt." : "al reato ex Art."} ${reatoEmergenti}, l'esame viene immediatamente interrotto ai sensi dell'art. 63, comma 1, c.p.p. ------//</b>`);
  ...
  // Firme
  html += renderSignatureBlock([`L'Interessat${F?'a':'o'}`, bloccoVerbalizzanti()]);
