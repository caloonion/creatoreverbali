...
import { $, renderHeader, getSoggetto, renderSignatureBlock, bloccoVerbalizzanti } from '../../core/utils.js';
...
export function generaIspezione(getOperantiListFn){
  ...
  html += pj(`Il giorno ${dataVerbale}, in ${luogoStesura}, alle ore ${oraStesura}, ${conc("il sottoscritto", "noi sottoscritti")} ${qualificaPG("sigla")} ${operanti}, ${conc("effettivo al", "effettivi al")} reparto in intestazione, ${conc("do atto", "diamo atto")} che alle ore ${oraInterv} del ${dataVerbale} in ${luogoInterv}, nel corso di una operazione di polizia tesa alla prevenzione ed alla repressione del traffico illecito di sostanze stupefacenti o psicotrope, avvalendoci del disposto di cui tratta la norma rubricata, ${conc("ho proceduto", "abbiamo proceduto")} in via d'urgenza all'ISPEZIONE:`);
  ...
  // La dichiarazione sulla difesa usa la formula condivisa da tutti i verbali,
  // impaginata come gli altri paragrafi.
  html += pj(testoDifensore("insp"));
  ...
  html += renderSignatureBlock(["La Persona Sottoposta ad Ispezione", bloccoVerbalizzanti()]);
