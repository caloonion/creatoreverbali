...
import { conc } from '../../core/militari.js';
...
export function generaNotifica(getOperantiListFn){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}${presso ? `, presso <b>${presso}</b>` : ""}, ${conc("il sottoscritto", "i sottoscritti")} <b>${operanti}</b>, ${conc("appartenente al", "appartenenti al")} Reparto in intestazione, ${conc("d\u00e0 atto", "danno atto")} di aver notificato l'atto di seguito indicato:`);
  ...
  html += renderSignatureBlock([
    // Chi riceve l'atto è il notificato, non un "consegnatario".
    F ? "La Notificata" : "Il Notificato",
    bloccoVerbalizzanti()
  ]);
