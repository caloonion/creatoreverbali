...
import { $, renderHeader, getSoggetto, getDocTipo, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti, splitItemsList, joinItemsWithSemicolons, getTipoDocumento } from '../../core/utils.js';
import { conc, qualificaPG } from '../../core/militari.js';
import { getFattispeciePerqL152 } from '../perql152/perql152.fattispecie.js';
import { getReatiFormattati } from './reati.lista.js';
...
export function generaVerbale161(getOperantiListFn){
  ...
  // Le fattispecie contestate sono un elenco: ciascuna porta con sé la propria
  // ipotesi di tentativo. Se il 161 accompagna un'altra pratica e qui non è
  // stato indicato nulla, si eredita l'elenco della pratica di provenienza.
  let reatiElenco = getReatiFormattati("v161");
  if(!reatiElenco.length && document.getElementById("sit_interruzione")?.checked){
    reatiElenco = getReatiFormattati("sit");
  }
  if(!reatiElenco.length && document.getElementById("pl152_gen_161")?.checked){
    const f = getFattispeciePerqL152();
    if(f) reatiElenco = [f];
  }
  const reato = reatiElenco.length
    ? joinItemsWithSemicolons(reatiElenco)
    : "________________________________________________";
  const piuReati = reatiElenco.length > 1;

  // Luogo e data del reato sono ormai proposti direttamente nei campi
  // (vedi precompilaLuogoDataReato): qui si legge semplicemente ciò che vi
  // compare, senza sostituzioni nascoste.
  const luogoReato = (document.getElementById("v161_luogo_reato")?.value || "").trim()
    || "________________________________________________";

  // Fatto contestato: se il 161 accompagna la perquisizione ex art. 4 e qui
  // non è stato indicato nulla, si riprende quanto rinvenuto.
  ...
  const avvNome = [
    (document.getElementById("v161_avv_nome")?.value || "").trim(),
    (document.getElementById("v161_avv_cognome")?.value || "").trim()
  ...
  const dom2PersonaDoc = getTipoDocumento("v161_dom2_persona_doc") || "________________________________";
  const dom2PersonaDocNum = (document.getElementById("v161_dom2_persona_doc_num")?.value || "").trim();
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, presso: ${luogoVerbale}, avanti ${conc("al sottoscritto", "ai sottoscritti")} ${qualificaPG("estesa")} ${operanti}, ${conc("effettivo", "effettivi")} al predetto reparto, è presente:`);
  ...
  // Il fatto è "commesso" oppure "accertato": nel verbale compare solo la
  // dizione scelta, con il proprio luogo e la propria data.
  const accertato = document.getElementById("v161_accertato")?.checked === true;
  const dataReato = (document.getElementById("v161_data_reato")?.value || "").trim() || "____________";

  html += pj(`La persona presente viene quindi avvertita che sono in corso indagini preliminari nei suoi confronti in ordine ${piuReati ? "ai reati ex Artt." : "al reato ex Art."} <b>${reato}</b>, ${accertato ? (piuReati ? "accertati" : "accertato") : (piuReati ? "commessi" : "commesso")} in <b>${luogoReato}</b>, in data <b>${dataReato}</b>; sinteticamente si contesta il seguente fatto: <b>${fatto}</b>, e che in relazione a dette indagini potrà avere ulteriori informazioni presso l'autorità procedente.`);
  ...
  html += pj((() => {
    // La dichiarazione è resa in prima persona e riportata fra virgolette:
    // va quindi chiusa dal punto fermo, come ogni frase compiuta.
    const testo = (() => {
  ...
  if(domTipo === "ufficio_studio") {
    ...
    if(accettaUfficioNo) {
      const dom2Text = (() => {
        if(dom2Tipo==="abitazione") return `presso la propria abitazione, al seguente indirizzo: <b>${dom2Ind}</b>`;
        if(dom2Tipo==="lavoro") return `presso il proprio luogo di lavoro, al seguente indirizzo: <b>${dom2Ind}</b>`;
        if(dom2Tipo==="persona") return `presso la seguente persona <b>${dom2PersonaNome}</b>, identificata dalla P.G. mediante <b>${dom2PersonaDoc}${dom2PersonaDocNum ? ` nr. ${dom2PersonaDocNum}` : ""}</b>`;
  ...
  html += renderSignatureBlock([F ? "L'Indagata" : "L'Indagato", bloccoVerbalizzanti()]);
