...
export function generaIspezione(getOperantiListFn){
  ...
  // Vengono nominati solo i tipi di ispezione effettivamente eseguiti e il
  // solo esito verificatosi: l'alternativa scartata non compare.
  const tipiIspezione = [];
  if(tipoPers) tipiIspezione.push("di effetti personali, indumenti e bagagli di persona");
  if(tipoVeic) tipiIspezione.push("veicolare");
  if(tipoLocale) tipiIspezione.push("locale");
  const tipiTesto = tipiIspezione.length
    ? tipiIspezione.join(" e ")
    : "____________________________";

  html += pj(`<b>VERBALE DI ISPEZIONE</b> ${tipiTesto}, ex art. 103, comma 2°, D.P.R. 09.10.1990 n° 309, conclusasi con esito <b>${esitoPos ? "POSITIVO" : "NEGATIVO"}</b>, eseguita nei confronti di:`);
  ...
  const oggetti = [];
  if(tipoPers) oggetti.push(`&mdash; Degli effetti personali, indumenti e bagagli della persona in rubrica generalizzata, ma non del suo corpo fisico;`);
  if(tipoVeic) oggetti.push(`&mdash; Del mezzo di trasporto marca ${veicMarca || "____"} modello ${veicModello || "____"} targa/telaio ${veicTarga || "____"};`);
  if(tipoLocale) oggetti.push(`&mdash; Altro ${localeCosa || "____"}, sopra generalizzato, ubicata in ${localeDove || "____"};`);
  if(oggetti.length){
  ...
  const dettaglioAvv = (difTipo === "avv_fiducia" || difTipo === "avv_ufficio")
    ? ` Avv. <b>${avvNome || "_____________________________"}</b>, del Foro di <b>${avvForo || "________________"}</b>, con Studio Legale in <b>${avvStudio || "_______________________"}</b>, <b>${avvVia || "_______________________"}</b>, tel. <b>${avvTel || "_________"}</b>, cell. <b>${avvCell || "_________"}</b>, fax <b>${avvFax || "_________"}</b>, il quale ${difIntervenuto
        ? `interveniva alle successive ore <b>${difOra}</b> e dichiarava: "${difDich || "________________________________"}"`
        : `non interveniva`}`
    : "";
  const dettaglioPersFiducia = (difTipo === "persona_fiducia") ? ` <b>${persFiduciaNome || "_____________________________"}</b>` : "";

  // Le tre dichiarazioni possibili si escludono a vicenda: nel verbale resta
  // solo quella effettivamente resa.
  const dichiarazioneDifesa = (difTipo === "avv_fiducia" || difTipo === "avv_ufficio")
    ? `Di nominare a suo favore un avvocato ${difTipo === "avv_fiducia" ? "di fiducia" : "d'Ufficio"}:${dettaglioAvv}.`
    : (difTipo === "persona_fiducia")
    ? `Di nominare a suo favore quale persona di fiducia il signor:${dettaglioPersFiducia}.`
    : `Di non ritenere necessaria la presenza di alcun difensore o persona di fiducia a suo favore durante l'esecuzione dell'atto.`;

  html += `
    <div style="margin:2pt 0 2pt 15pt; ${pJust}">${dichiarazioneDifesa}</div>
  `;
