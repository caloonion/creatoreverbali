...
export function generaPerquisizione(getOperantiListFn){
  ...
  const colonne = ["La Parte", "I Verbalizzanti"];
...
// 6. GENERATORE ETICHETTA REPERTO
export function generaEtichetta(){
  const dati = {
    legione: $("legione").value,
    comando: $("comando").value,
    squadra: $("squadra").value
  ...
  const posizione = (document.getElementById("etichetta_posizione")?.value || "").trim();
  const repertante = (document.getElementById("etichetta_repertante")?.value || "").trim();
      ...
      <table style="width:100%; border-collapse:collapse; font-size:9.5pt; margin-top:2mm;">
        ...
        <tr>
          <td colspan="2" style="${cell}">
            <b>OGGETTO:</b> ${sostDesc || "____________________________________________"}
          </td>
        </tr>
        <tr>
          <td colspan="2" style="${cell}">
            <b>GENERALITÀ ${F ? "TRASGREDITRICE" : "TRASGRESSORE"}:</b> <b>${s1.boldName}</b>, ${s1.dati}
          </td>
        </tr>
        <tr>
          <td colspan="2" style="${cell}">
            <b>BREVE DESCRIZIONE DEL REPERTO:</b> ${sostDesc || "____________________________________________"}${luogoInterv ? `, rinvenuto in data ${dataVerbale || "____"}, in ${luogoInterv}` : ""}.
          </td>
        </tr>
        <tr>
          <td colspan="2" style="${cell}">
