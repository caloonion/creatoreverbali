...
import { $, attachDateMask, setDocAltroVisibility, setAltroDocumentoVisibility, isVerbaleInUffici, linkFieldsBidirectional, splitItemsList, joinItemsWithSemicolons, getTipoDocumento, descrittoreDocumento, proponiValore, riabilitaProposta } from './core/utils.js';
...
import { initListeReati, setCallbackReati, getReati, setReati, ripristinaReatiDaJson, aggiungiReato } from './verbali/art161/reati.lista.js';
import { ISTANZE_161, istanzaDelDocumento, getIstanzaAttiva, attivaIstanza161, conIstanza161, modificaIstanza161, azzeraIstanze161, serializzaIstanze161, ripristinaIstanze161 } from './verbali/art161/istanze.js';
import { popolaMenuFori } from './verbali/art161/fori.js';
import { syncDifensoreUI, initFioriDifensore } from './core/difensore.js';
import { generaTrasmissionePatente } from './verbali/patente223/patente223.trasmissione.js';
import { capitalizzaNome, normalizzaComune, normalizzaCivico, maiuscolo, normalizzaIndirizzo } from './core/formattazione.js';
import { initRiordinoSegnalibri, aggiornaOrdineSegnalibri, azzeraOrdineSegnalibri } from './core/segnalibri.js';
import { setGruppoFirma, syncGruppoFirmaUI } from './core/gruppo-firma.js';
...
function setDocAttivo(doc){
  // I tre verbali ex art. 161 condividono il modulo di compilazione ma sono
  // atti distinti: prima di mostrarne uno si ripone quello in lavorazione.
  const ist = istanzaDelDocumento(doc);
  if(ist) attivaIstanza161(ist);
...
function allineaFattispecieL152(){
  // La fattispecie scelta nella perquisizione ex art. 4 viene riportata come
  // prima riga dell'elenco del 161, dove resta modificabile.
  if(document.getElementById("pl152_gen_161")?.checked !== true) return;
  const testo = getFattispeciePerqL152();
  if(!testo) return;
  setReati("v161", [{ testo, tentato: false }]);
}


// Verbale ex art. 161: luogo e data del reato coincidono di norma con quelli
// dell'intervento. Vengono quindi proposti direttamente nei campi — non solo
// nell'anteprima — così chi compila da telefono vede che quella parte è già
// pronta, e resta libero di modificarla.
function precompilaLuogoDataReato(){
  const box = document.getElementById("container_art161");
  if(!box || box.style.display === "none") return;

  // Valori proposti, non imposti: appena l'operatore mette mano al campo il
  // sito smette di intervenire. Senza questa regola, cancellando l'ultimo
  // carattere il valore verrebbe reinserito e il campo sarebbe inutilizzabile.
  const comune = (document.getElementById("verbale_comune")?.value || "").trim();
  if(comune) proponiValore(document.getElementById("v161_luogo_reato"), normalizzaComune(comune));

  const dataVerbale = (document.getElementById("dataVerbale")?.value || "").trim();
  if(dataVerbale) proponiValore(document.getElementById("v161_data_reato"), dataVerbale);
}
...
function buildPreview(){
  try {
    ...
    const html = istanzaDelDocumento(docAttivo)
      ? conIstanza161(istanzaDelDocumento(docAttivo), () => generaVerbale161(getOperantiList))
...
function stampaTutto(){
  ...
  if(currentPratica === "sit") {
    ...
    if(document.getElementById("sit_interruzione")?.checked && document.getElementById("sit_gen_161")?.checked){
      parts.push(conIstanza161("sit", () => generaVerbale161(getOperantiList)));
    }
  } else if(currentPratica === "161") {
    parts.push(conIstanza161("principale", () => generaVerbale161(getOperantiList)));
    ...
    if(document.getElementById("pl152_gen_161")?.checked) parts.push(conIstanza161("pl152", () => generaVerbale161(getOperantiList)));
...
function resetAllFields(preserveStructure = false){
  ...
  // Gli elenchi dei reati sono righe costruite a runtime, prive di
  // identificativo: il ciclo qui sopra non le vede, quindi vanno svuotate
  // esplicitamente come già si fa per sostanze e domande delle S.I.T.
  svuotaElenchiReati();
  azzeraIstanze161();

  // I campi con valore proposto tornano disponibili: dopo un azzeramento la
  // proposta automatica deve poter ripartire.
  document.querySelectorAll("[data-valore-proposto], [data-modificato-a-mano]")
    .forEach(riabilitaProposta);
  ...
  if(preserveStructure){
    ...
    // "Nuovo" azzera anche la pattuglia: resta il solo militare identificato
    // dal codice di accesso. Col codice generico non resta nessuno.
    const identificato = sessionStorage.getItem("v75_user_operante");
    setUserOperanteFirst(identificato || null, addOperanteSelect);
    chiudiTuttePratiche();
...
// S.I.T. con interruzione: il reato per cui emergono indizi confluisce nei
// campi del verbale ex art. 161 c.p.p., dove resta modificabile. I due campi
// restano allineati finché non li si scrive in modo diverso, come già avviene
// fra perquisizione e sequestro.
function collegaReatoSitCon161(){
  // S.I.T. con interruzione: i reati per cui emergono indizi e quelli del
  // verbale ex art. 161 che ne consegue sono gli stessi atti. I due elenchi
  // restano allineati in entrambe le direzioni, e la copia avviene sui campi
  // e non solo nell'anteprima: chi compila da telefono deve vedere che la
  // parte collegata risulta già compilata.
  const attivo = () => document.getElementById("sit_interruzione")?.checked === true
                    && document.getElementById("sit_gen_161")?.checked === true;
  ...
  const allinea = () => {
    ...
    inCorso = true;
    try {
      if(getIstanzaAttiva() === "sit"){
        // Si sta compilando il 161 da S.I.T.: comanda quello.
        if(!uguali(getReati("v161"), getReati("sit"))) setReati("sit", getReati("v161"));
      } else {
        // Altrimenti comanda l'elenco delle S.I.T.
        modificaIstanza161("sit", () => {
          if(!uguali(getReati("sit"), getReati("v161"))) setReati("v161", getReati("sit"));
        });
      }
    } finally {
      inCorso = false;
  ...
  setCallbackReati(() => { allinea(); buildPreview(); });

  ["sit_interruzione","sit_gen_161"].forEach(id => {
    document.getElementById(id)?.addEventListener("change", allinea);
  });
}

// Verbali che prevedono la sezione difensore/persona di fiducia.
const PREFISSI_DIFENSORE = ["insp","perq","p352","pl152","sqv"];


// Rimozione di cadavere: quando il verbale non è redatto presso gli Uffici, il
// luogo dell'intervento coincide con quello di stesura. I due campi restano
// quindi allineati in entrambe le direzioni, e si scollegano solo se si
// dichiara che il verbale è stato redatto in caserma.
function collegaLuogoCadavere(){
  const COPPIE = [["verbale_via", "cad_via"], ["verbale_comune", "cad_comune"]];
  const inUffici = () => isVerbaleInUffici();

  COPPIE.forEach(([a, b]) => {
    const ea = document.getElementById(a);
    const eb = document.getElementById(b);
    if(!ea || !eb) return;
    // Il valore viene copiato già normalizzato (comune con la sigla, via con
    // le iniziali maiuscole), così i due campi restano davvero identici anche
    // quando la normalizzazione scatta dopo la copia.
    const normalizza = (el, valore) => {
      if(el.hasAttribute("data-comune")) return normalizzaComune(valore);
      if(el.hasAttribute("data-indirizzo")) return normalizzaIndirizzo(valore);
      return valore;
    };
    const copia = (da, a2) => () => {
      if(inUffici()) return;
      const valore = normalizza(a2, da.value);
      if(a2.value === valore) return;
      a2.value = valore;
      buildPreview();
    };
    ["input", "blur"].forEach(ev => {
      ea.addEventListener(ev, copia(ea, eb));
      eb.addEventListener(ev, copia(eb, ea));
    });
  });
}

// Verbali che contestano una o più fattispecie penali.
const PREFISSI_REATI = ["v161","sit","p352"];


// Ricostruisce gli elenchi dei reati dopo il ripristino di una bozza: nel
// salvataggio viaggiano come testo nel campo nascosto, qui tornano righe.

// Svuota gli elenchi dei reati di tutti i verbali che ne prevedono uno.
function svuotaElenchiReati(){
  PREFISSI_REATI.forEach(prefix => setReati(prefix, []));
}

function ripristinaElenchiReati(){
  PREFISSI_REATI.forEach(ripristinaReatiDaJson);
}
...
function wireEvents(){
  ...
  ["veh_no","veh_yes","veh_owner_same","veh_prop_sesso","ritiro_patente","invito_patente","verbale_180","extracomunitario"].forEach(id=>{
  ...
  [
    "v161_lingua_si","v161_lingua_no","v161_avv_foro",
  ...
  [
    "p352_tipo_pers","p352_tipo_locale","p352_esito_pos","p352_esito_neg","p352_gen_sequestro"
  ].forEach(id=>{
    document.getElementById(id)?.addEventListener("change", ()=>{ syncPerq352UI(); buildPreview(); });
  });

  [
    "pl152_veic_esteso","pl152_veic_qualita","pl152_esito_pos","pl152_esito_neg","pl152_gen_sequestro"
  ].forEach(id=>{
  ...
  [
    "sqv_prop_diverso","sqv_prop_sesso",
    "sqv_cc","sqv_cdp","sqv_rca","sqv_cust_sesso","sqv_cust_area"
  ].forEach(id=>{
  ...
  document.getElementById("pl152_161_reato_tipo")?.addEventListener("change", ()=>{ syncPerqL152UI(); allineaFattispecieL152(); buildPreview(); });
  document.getElementById("pl152_161_reato")?.addEventListener("input", ()=>{ allineaFattispecieL152(); buildPreview(); });
  document.getElementById("pl152_gen_161")?.addEventListener("change", ()=>{ allineaFattispecieL152(); });

  ["v161_commesso","v161_accertato","v161_tentativo","v161_avv_foro",
   "sit_tentativo","p352_tentativo","pl152_tentativo"].forEach(id => {
    document.getElementById(id)?.addEventListener("change", buildPreview);
  });
  ["sit_reato_cerca","p352_reato_cerca"].forEach(id => {
    document.getElementById(id)?.addEventListener("input", buildPreview);
  });
  document.getElementById("v161_data_reato")?.addEventListener("input", buildPreview);
  ...
  // Le altre normalizzazioni seguono lo stesso schema: il campo viene corretto
  // quando lo si lascia, così quello che si vede nel modulo è esattamente
  // quello che finisce nel verbale.
  // Il suggerimento del campo numero segue il documento scelto: per il
  // fotosegnalamento non si indica un numero ma il Codice CUI.
  const COPPIE_DOC = [
    ["s1_doc_tipo", "s1_doc_num"],
    ["s2_doc_tipo", "s2_doc_num"],
    ["aff_doc", "aff_doc_num"],
    ["vei_inc_doc", "vei_inc_doc_num"],
    ["rim_cust_doc", "rim_cust_doc_num"],
    ["v161_dom2_persona_doc", "v161_dom2_persona_doc_num"]
  ];
  function aggiornaEtichettaNumeroDocumento(){
    COPPIE_DOC.forEach(([idTipo, idNum]) => {
      const num = document.getElementById(idNum);
      if(!num) return;
      const tipo = getTipoDocumento(idTipo);
      num.placeholder = descrittoreDocumento(tipo).fotosegnalamento ? "Codice CUI" : "nr. documento";
    });
  }
  COPPIE_DOC.forEach(([idTipo]) => {
    document.getElementById(idTipo)?.addEventListener("change", aggiornaEtichettaNumeroDocumento);
  });
  aggiornaEtichettaNumeroDocumento();

  // I menu "tipo documento" aggiuntivi mostrano il campo libero solo su "Altro".
  ["aff_doc","vei_inc_doc","rim_cust_doc","v161_dom2_persona_doc"].forEach(id => {
    const sel = document.getElementById(id);
    if(!sel) return;
    sel.addEventListener("change", () => { setAltroDocumentoVisibility(id); buildPreview(); });
    setAltroDocumentoVisibility(id);
  });
  ["aff_doc_altro","vei_inc_doc_altro","rim_cust_doc_altro","v161_dom2_persona_doc_altro",
   "vei_inc_doc_num","rim_cust_doc_num","v161_dom2_persona_doc_num"].forEach(id => {
    document.getElementById(id)?.addEventListener("input", buildPreview);
  });

  const NORMALIZZATORI = [
    ["data-cognome",   maiuscolo],
    ["data-maiuscolo", maiuscolo],
    ["data-civico",    normalizzaCivico],
    ["data-indirizzo", normalizzaIndirizzo]
  ];
  NORMALIZZATORI.forEach(([attr, fn]) => {
    document.querySelectorAll(`input[${attr}]`).forEach(el => {
      el.addEventListener("blur", () => {
        const formattato = fn(el.value);
        if(formattato !== el.value){ el.value = formattato; buildPreview(); }
      });
    });
  });
  ...
  const ids = [
    ...
    "veh_marca","veh_modello","veh_targa",
    "veh_prop_cognome","veh_prop_nome","veh_prop_nato_a","veh_prop_nato_il",
    "veh_prop_res_comune","veh_prop_res_via","veh_prop_res_civ",
    ...
    "sit_fatti","sit_adr",
    "v161_lingue","v161_luogo_reato","v161_fatto",
    ...
    "perq_ora_inizio","perq_ora_fine","perq_circostanze",
    ...
    "p352_luoghi","p352_motivo","p352_ora_fine","p352_rinvenuto",
    ...
    "pl152_ora_fine","pl152_rinvenuto",
    ...
    "cad_via","cad_comune","cad_luogo","cad_medico","cad_causa_testo","cad_epoca_testo","cad_pm","cad_pm_ora",
    ...
    "v161_reato_cerca","v161_data_reato","sit_reato_cerca","p352_reato_cerca",
    "v161_reati_json","sit_reati_json","p352_reati_json",
    "insp_dif_tipo","insp_dif_avv_nome","insp_dif_avv_foro","insp_dif_avv_studio","insp_dif_avv_via","insp_dif_avv_tel","insp_dif_avv_cell","insp_dif_avv_fax","insp_dif_intervenuto_ora","insp_dif_dichiarazione","insp_pers_fiducia_nome","perq_dif_tipo","perq_dif_avv_nome","perq_dif_avv_foro","perq_dif_avv_studio","perq_dif_avv_via","perq_dif_avv_tel","perq_dif_avv_cell","perq_dif_avv_fax","perq_dif_intervenuto_ora","perq_dif_dichiarazione","perq_pers_fiducia_nome","p352_dif_tipo","p352_dif_avv_nome","p352_dif_avv_foro","p352_dif_avv_studio","p352_dif_avv_via","p352_dif_avv_tel","p352_dif_avv_cell","p352_dif_avv_fax","p352_dif_intervenuto_ora","p352_dif_dichiarazione","p352_pers_fiducia_nome","pl152_dif_tipo","pl152_dif_avv_nome","pl152_dif_avv_foro","pl152_dif_avv_studio","pl152_dif_avv_via","pl152_dif_avv_tel","pl152_dif_avv_cell","pl152_dif_avv_fax","pl152_dif_intervenuto_ora","pl152_dif_dichiarazione","pl152_pers_fiducia_nome","sqv_dif_tipo","sqv_dif_avv_nome","sqv_dif_avv_foro","sqv_dif_avv_studio","sqv_dif_avv_via","sqv_dif_avv_tel","sqv_dif_avv_cell","sqv_dif_avv_fax","sqv_dif_intervenuto_ora","sqv_dif_dichiarazione","sqv_pers_fiducia_nome",
    "aff_doc","aff_doc_altro","vei_inc_doc","vei_inc_doc_altro","vei_inc_doc_num",
    "rim_cust_doc","rim_cust_doc_altro","rim_cust_doc_num",
    "v161_dom2_persona_doc","v161_dom2_persona_doc_altro","v161_dom2_persona_doc_num"
  ];
...
function initApp() {
  ...
  popolaMenuFori("v161_avv_foro");
  collegaLuogoCadavere();

  // Elenchi dei reati: ogni riga porta con sé la propria ipotesi di tentativo.
  initListeReati(PREFISSI_REATI);
  // Il collegamento fra S.I.T. e 161 registra la propria callback sugli
  // elenchi: va quindi stabilito dopo averli inizializzati, altrimenti
  // verrebbe sovrascritto.
  collegaReatoSitCon161();

  // Sezioni difensore: struttura e testo sono condivisi da tutti i verbali,
  // quindi si collegano in un ciclo solo.
  initFioriDifensore(PREFISSI_DIFENSORE);
  PREFISSI_DIFENSORE.forEach(pref => {
    [`${pref}_dif_tipo`, `${pref}_dif_intervenuto`].forEach(id => {
      document.getElementById(id)?.addEventListener("change", () => { syncDifensoreUI(pref); buildPreview(); });
    });
    [`${pref}_dif_avv_nome`,`${pref}_dif_avv_foro`,`${pref}_dif_avv_studio`,`${pref}_dif_avv_via`,
     `${pref}_dif_avv_tel`,`${pref}_dif_avv_cell`,`${pref}_dif_avv_fax`,
     `${pref}_dif_intervenuto_ora`,`${pref}_dif_dichiarazione`,`${pref}_pers_fiducia_nome`].forEach(id => {
      const el = document.getElementById(id);
      if(!el) return;
      el.addEventListener(el.tagName === "SELECT" ? "change" : "input", buildPreview);
    });
    syncDifensoreUI(pref);
  });

  // Gruppo firma: la scelta resta memorizzata sul dispositivo.
  syncGruppoFirmaUI();
  document.getElementById("gruppoFirmaSelect")?.addEventListener("change", (ev) => {
    setGruppoFirma(ev.target.value);
    syncGruppoFirmaUI();
    buildPreview();
  });
  ...
  const syncHelpers = {
    ...
    syncCadavereUI,
    syncEtichetteSecondarie,
    // Le righe dei reati vanno ricostruite dal campo nascosto ripristinato.
    ripristinaElenchiReati,
    syncPerquisizioneUI,
    syncNarcotestUI,
    syncEtichettaUI,
    setDocAttivo,
