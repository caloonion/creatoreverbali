...
export function generaSeqVeicolo(getOperantiListFn){
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}, ${conc("il sottoscritto", "noi sottoscritti")} <b>${operanti}</b>, ${qualificaPG("breve")} in servizio presso l'ufficio di cui all'intestazione, ${conc("rendo noto", "rendiamo noto")} a chi di dovere perch&eacute; consti che in data e luogo di cui sopra, a seguito di sinistro stradale con esito <b>${sinEsito || "____________________"}</b>, verificatosi in data <b>${sinData || "____________"}</b> alle ore <b>${sinOra || "______"}</b>, in localit&agrave; <b>${sinLocalita || "____________________"}</b> nel comune di <b>${sinComune || "____________________"}</b>, ${conc("ho proceduto", "abbiamo proceduto")} al sequestro del veicolo indicato in quanto direttamente coinvolto nell'evento infortunistico.`);
