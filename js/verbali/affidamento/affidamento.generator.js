...
import { $, renderHeader, getSoggetto, renderSignatureBlock, bloccoVerbalizzanti, getTipoDocumento , descrittoreDocumento} from '../../core/utils.js';
...
function getAffidatario(){
  ...
  const doc = getTipoDocumento("aff_doc");
  ...
  if(resVia || resCiv) dati += `${resVia} ${resCiv}`.trim() + ", ";
  if(doc || docNum){
    const d = descrittoreDocumento(doc);
    dati += `identificat${isFemale ? "a" : "o"} mediante ${doc || "documento"} ${d.etichettaNumero} ${docNum || "____________"}`;
    if(docDa) dati += ` ${descrittoreDocumento(doc).rilasciato} da ${docDa}`;
...
export function generaAffidamento(getOperantiListFn){
  ...
  const luogoRedazione = (luogoVerbaleComune || luogoVerbaleVia)
    ? `in ${luogoVerbaleComune || "______________________"}${luogoVerbaleVia ? `, ${luogoVerbaleVia}` : ""}`
    : `negli Uffici del Comando in intestazione`;

  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoRedazione}, ${conc("il sottoscritto", "noi sottoscritti")} ${qualificaPG("breve")} <b>${operanti}</b>, ${conc("appartenente al", "appartenenti al")} Reparto in epigrafe, avuta la presenza del/della Sig./ra <b>${aff.boldName}</b>, sopra meglio generalizzat${aff.isFemale ? "a" : "o"}, al/alla quale <b>viene affidat${M ? "a" : "o"}</b> il minore sopra identificat${M ? "a" : "o"}, fermat${M ? "a" : "o"} ed identificat${M ? "a" : "o"} in data odierna alle ore <b>${fermoOra || "______"}</b> circa, nel Comune di <b>${fermoComune || "____________________"}</b>${fermoLoc ? `, localit\u00e0 <b>${fermoLoc}</b>` : ""}${fermoVia ? `, via <b>${fermoVia}</b>${fermoCiv ? ` nr. <b>${fermoCiv}</b>` : ""}` : ""}${fermoPresso ? `, presso <b>${fermoPresso}</b>` : ""}.`);
  ...
  html += renderSignatureBlock([aff.isFemale ? "L'Affidataria" : "L'Affidatario", bloccoVerbalizzanti()]);
