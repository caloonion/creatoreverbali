...
import { $, renderHeader, getSoggetto, getLuogoVerbaleText, renderSignatureBlock, bloccoVerbalizzanti, splitItemsList, joinItemsWithSemicolons, getTipoDocumento } from '../../core/utils.js';
...
export function generaRimozione(getOperantiListFn){
  ...
  const custDoc = getTipoDocumento("rim_cust_doc");
  const custDocNum = val("rim_cust_doc_num");
  ...
  html += pj(`Il giorno <b>${dataVerbale}</b>, alle ore <b>${oraVerbale}</b>, ${luogoVerbale}, ${conc("il sottoscritto", "noi sottoscritti")} ${qualificaPG("breve")} <b>${operanti}</b>, ${conc("appartenente al", "appartenenti al")} Comando di cui all'intestazione, ${conc("ho proceduto", "abbiamo proceduto")} alla rimozione del veicolo sopra indicato, risultato di propriet&agrave; di ${propDiverso && propNome
